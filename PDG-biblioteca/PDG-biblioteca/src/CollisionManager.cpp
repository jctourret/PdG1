...
bool CollisionManager::CheckCollisionAndReact(Shape* shape, ReSprite* sprite, bool firstOneReacts)
{
	a.width = shape->width;
	a.height = shape->height;
	a.pos = shape->getPosition();
	...
	if (isColliding())
	{
		return true; //aca hacer un switch para ver a donde mover el shape
	}
}
